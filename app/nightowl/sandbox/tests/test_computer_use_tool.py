...
Auto-creates a computer_use container via ensure_container() on first use.
"""
...
def _mock_sandbox_mgr(stdout: str = '{"success": true}', stderr: str = "", exit_code: int = 0) -> MagicMock:
    mgr = MagicMock()
    mgr.ensure_container = AsyncMock(return_value="container:cu1")
...
class TestComputerUseScreenshot:
    async def test_returns_screenshot_data(self, manager: SessionManager):
        ...
        sandbox_mgr = _mock_sandbox_mgr(stdout="base64_screenshot_data_here")
...
class TestComputerUseAction:
    async def test_click_action_with_coords(self, manager: SessionManager):
        ...
        with patch("nightowl.sandbox.computer_use_tool.verify_risk", new_callable=AsyncMock) as mock_v:
            mock_v.return_value = {"verified_risk": RiskLevel.LOW, "reasoning": "ok"}
            await computer_use_action(
                ctx, action="click", coords=[500, 300],
                risk_level="low", risk_justification="click button",
            )
    ...
    async def test_type_action_with_text(self, manager: SessionManager):
        ...
        with patch("nightowl.sandbox.computer_use_tool.verify_risk", new_callable=AsyncMock) as mock_v:
            ...
            await computer_use_action(
                ...
                risk_level="low", risk_justification="type text",
            )
